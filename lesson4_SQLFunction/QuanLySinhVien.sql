...
-- Tinh diem trung binh cac mon hoc cua moi hoc vien
select s.StudentID, s.StudentName, avg(mark) as 'DTB'
from Student s join Mark m on s.StudentID = m.StudentID
group by s.StudentID, s.StudentName;
-- Hien thi nhung ban hoc vien co diem trung binh cac mon hoc lon hon 15
select s.StudentID, s.StudentName, avg(mark) as 'DTB'
from Student s join Mark m on s.StudentID = m.StudentID
group by s.StudentID, s.StudentName
having avg(mark) > 15;
-- Hien thi thong tin cac hoc vien co diem trung binh lon nhat
select s.StudentID, s.StudentName, avg(mark) as 'DTB lon nhat'
from Student s join Mark m on s.StudentID = m.StudentID
group by s.StudentID, s.StudentName
having avg(mark) >= all (select avg(mark) from Mark group by Mark.StudentID);
-- Hien thi tat ca cac thong tin mon hoc co credit lon nhat
select * from Subject where credit = (select max(credit) from Subject); 
-- Hien thi cac thong tin mon hoc co diem thi lon nhat
...
select s.StudentID, s.StudentName, round(avg(mark),2) as 'DTB'
from Student s
